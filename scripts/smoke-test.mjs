#!/usr/bin/env node
/**
 * End-to-end check of a built `majin`.
 *
 * Starts the binary on a spare port, drives the real HTTP and WebSocket
 * surface, and runs a command in a real shell. This covers the seams the Rust
 * unit tests cannot: cookie handling, the origin check, the WebSocket close
 * codes, and whether the frontend actually made it into the binary.
 *
 *   node scripts/smoke-test.mjs [--binary path] [--port 8991]
 */
...
const BINARY = argValue('binary', './target/release/majin');
...
try {
  if (!(await waitForServer())) {
    console.error(`${BINARY} never started listening on ${PORT}`);
    process.exit(1);
  }
